...
{If no Tier 1 stocks:}
**No stocks in Tier 1 today.**

{For each Tier 1 stock:}
### {TICKER} - {Company name from notes}
**Current:** ${price} | **Entry:** ${low}-${high} | **Status:** {status} ✅/⚠️
**Distance:** {distance_text}
**RSI:** {rsi} | **Risk:** {risk_level} | **Earnings:** {date}

{Intraday strategy from Step 6}

**Notes from CSV:** {highlight any warnings from notes field}

**Recent Developments:** {any news from daily update past 24-48 hours}
...
{List stocks with brief reasoning:}
- **{TICKER}**: {status} - {reason (overbought, too extended, extreme risk)}
...
⚠️  **Sector Context:** {If available from portfolio summary}

⚠️  **Data Freshness:** Based on latest available daily updates. For real-time pre-market prices, consider running `/deep-stock-research --type=daily-update` first.
...
### Issue: "Auto-fetch taking too long"
**Cause:** Fetching fresh data for large portfolios (8-16 minutes normal)
**Solution:**
- Wait for completion - progress indicators shown
- **Alternative:** Cancel and use stale data (not recommended for trading)
- **Prevention:** Run `/deep-stock-research` in evening for next morning

### Issue: "Some stocks failed to update"
**Cause:** Comet browser not running, network issues, or Perplexity rate limits
**Solution:**
- Check Comet is running with remote debugging (port 9223)
- Command continues with other stocks, failed ones shown in warning
- Manually update failed stocks later if needed

### Issue: "Data still shows as yesterday after auto-fetch"
**Cause:** Market closed (after 4:00 PM ET) or before pre-market (before 4:00 AM ET)
**Normal:** Yesterday's close is latest available data outside market hours
**Solution:** This is expected behavior - yesterday's data is acceptable for morning planning

### Issue: "Watch list takes 15+ minutes to generate"
**Cause:** Large portfolios with stale data trigger auto-fetch
**Solutions:**
- **Morning routine:** Run `/morning-watch-list` once in evening (caches data)
- **Manual control:** Run `/deep-stock-research` separately when you have time
- **Smaller portfolios:** Use `--portfolio=NAME` to process one at a time

### Issue: "Comet devtools not responding"
**Cause:** Comet browser not launched with remote debugging enabled
**Solution:**
```bash
...
```
Wait 3-5 seconds after launch before running command again.

### Issue: "All stocks show 'No data' even after auto-fetch"
**Cause:** Auto-fetch failed for all stocks, or month folder doesn't exist
**Solution:**
1. Check error messages during auto-fetch
2. Verify Comet browser is running
3. Check network connectivity
4. Manually run one stock to debug:
```bash
