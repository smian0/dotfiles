...
tags: [trading, stocks, discovery, research]
---
...
**Discovery Method:** Multi-round agentic research via Perplexity
**Research Rounds:** {NUMBER}
**Screening Criteria:**
- Market cap <${MAX_MARKET_CAP}
- Low analyst coverage (<10 analysts)
- Strong recent catalysts
- Reasonable liquidity (>100K volume)
- Sector alignment: {SECTOR}

**Top Picks:** {Brief summary of top 3-5}
...
### Tier 2: Watch List (Conditional Buy)
{Similar format for Tier 2 stocks}
...
### Round 1: Discovery
**Query:** {Summary of query}
**Results:** {XX} initial candidates identified

### Round 2: Financial Screening
**Criteria:** {List criteria}
**Results:** Narrowed to {XX} stocks

### Round 3: Catalyst Analysis
**Focus:** {What was analyzed}
**Results:** Top {XX} stocks with strong catalysts

### Round 4: Risk Assessment
**Method:** {How risks were evaluated}
**Results:** Final {XX} stocks selected

### Round 5: Entry Zones (Optional)
**Analysis:** {What was determined}
**Results:** Entry recommendations provided
...
|------|-------|-------|--------|
| {DATE} | {TICKER} | Earnings | High |
| {DATE} | {TICKER} | Product launch | Medium |
...
- Research based on Perplexity data as of {DATE}
- Market conditions can change rapidly
- Small/mid-cap stocks are inherently riskier
- Lower liquidity = wider bid-ask spreads
- Always verify financials independently

**Risk Warnings:**
- Hidden gems = higher volatility
- Analyst coverage is low (information risk)
- Company-specific risks apply
- Sector risks apply ({SECTOR}-specific)
...
- [TICKER1 Comprehensive Research](../{TICKER1}/comprehensive-research_{DATE}.md)
- [TICKER2 Comprehensive Research](../{TICKER2}/comprehensive-research_{DATE}.md)

**Sector Analysis:**
- [Related sector reports if exist]
...
**Generated:** {DATE} {TIME}
**Next Discovery Run:** {Recommended frequency, e.g., quarterly}
```

### Step 6: Create Portfolio CSV (Optional)

If user wants to track these stocks, create portfolio CSV:

**File Location**:
```
~/dotfiles/research/stocks/portfolios/{SECTOR}-hidden-gems.csv
```

**Format**:
```csv
ticker,entry_zone_low,entry_zone_high,next_earnings,risk_level,notes
TICKER1,XX,XX,YYYY-MM-DD,medium,Brief note from discovery
TICKER2,XX,XX,YYYY-MM-DD,high,Brief note from discovery
```

This enables user to immediately use `/deep-stock-research --portfolio={SECTOR}-hidden-gems --type=daily-update` and `/morning-watch-list` with discovered stocks.
...
```
✅ Hidden Gem Discovery Complete!

📁 Discovery Report:
~/dotfiles/research/stocks/2025-10/discoveries/quantum-computing-hidden-gems_2025-10-06.md

🎯 Top Recommendations:
1. TICKER1 - Company Name ($XXX M cap) - TIER 1 ⭐⭐⭐⭐⭐
2. TICKER2 - Company Name ($XXX M cap) - TIER 1 ⭐⭐⭐⭐⭐
3. TICKER3 - Company Name ($XXX M cap) - TIER 2 ⭐⭐⭐⭐

📊 Comprehensive Research Available:
- TICKER1: comprehensive-research_2025-10-06.md
- TICKER2: comprehensive-research_2025-10-06.md
- TICKER3: comprehensive-research_2025-10-06.md

📋 Portfolio CSV Created:
~/dotfiles/research/stocks/portfolios/quantum-computing-hidden-gems.csv

🔄 Next Steps:
1. Review discovery report
2. Read comprehensive research for Tier 1 stocks
3. Set alerts at recommended entry zones
4. Run daily updates: /deep-stock-research --portfolio=quantum-computing-hidden-gems --type=daily-update
```
...
### No Stocks Found
```
⚠️ No hidden gems found matching criteria in {SECTOR} sector.

Try:
1. Increase market cap limit: --market-cap-max=10B
2. Broaden sector definition
3. Check if sector name is correct
```

### Insufficient Data
```
⚠️ Perplexity returned incomplete data for some candidates.

Proceeding with available data but recommend:
- Manual verification of financials
- Check company investor relations directly
```

## Usage Examples

```bash
# Discover quantum computing hidden gems
/discover-stocks --sector=quantum-computing

# Discover AI chips with higher market cap limit
/discover-stocks --sector=AI-chips --market-cap-max=10B

# Find at least 8 neuromorphic stocks
/discover-stocks --sector=neuromorphic --min-stocks=8
```
...
**Custom sectors:** You can specify any sector, Perplexity will interpret it.

## Integration with Existing Workflow

Once stocks are discovered:

1. **Track daily**: `/deep-stock-research --portfolio={SECTOR}-hidden-gems --type=daily-update`
2. **Generate watch list**: `/morning-watch-list --portfolio={SECTOR}-hidden-gems`
3. **Update research**: `/deep-stock-research {TICKER} --type=comprehensive` (quarterly)

## Important Notes

- **Discovery is just the start** - Always run comprehensive research on top picks
- **Small-cap risk** - Hidden gems = higher volatility and risk
- **Verify independently** - Cross-check financials with company filings
- **Liquidity matters** - Some stocks may have wide bid-ask spreads
- **Sector correlation** - Hidden gems often move with sector trends
- **Patience required** - Entry zones may take weeks/months to reach
